...
def update_sensor(admin,password,company_api_key,location_id,sensor_id,sensor_name,sensor_category,sensor_meta):
    ...
    if comprobar is None:
        ...
        cursor_dos = db.cursor() # otro cursor
        statement_dos = "UPDATE sensor SET location_id = ?, sensor_id = ?, sensor_name = ?, sensor_category = ?, sensor_meta = ? WHERE location_id = ?"
        cursor_dos.execute(statement_dos, [location_id, sensor_id, sensor_name, sensor_category, sensor_meta, location_id])
        db.commit()
        return True
...
def get_sensor_data(sensor_api_key,desde,hasta):
    db = get_db()
    cursor = db.cursor()
    statement = "SELECT sensor_api_key, tiempo, variable_uno, variable_dos FROM sensor_datas WHERE sensor_api_key = ? AND tiempo BETWEEN ? AND ?"
...
def insert_sensor_data(sensor_api_key,tiempo,variable_uno,variable_dos):
    ...
    statement = "INSERT INTO sensor_datas(sensor_api_key, tiempo, variable_uno, variable_dos) VALUES (?, ?, ?, ?)"
